...
export const start_ripple = (event) => {
...
const set_element_size = (element) => {
  const width = Number(
    window.getComputedStyle(element).getPropertyValue('width').replace(/px/gi, '')
  );
  const height = Number(
    window.getComputedStyle(element).getPropertyValue('height').replace(/px/gi, '')
  );
  element_data.size = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)) / 4;
}

const set_color_and_transition = (element) => {
  element_data.color = window.getComputedStyle(element).getPropertyValue('--wk-rp-color');
...
const get_formula = () => {
  return {
    a: Math.sqrt(element_data.touch_data.bottom ** 2 + element_data.touch_data.right ** 2),
    b: Math.sqrt(element_data.touch_data.bottom ** 2 + element_data.touch_data.left ** 2),
    c: Math.sqrt(element_data.touch_data.top ** 2 + element_data.touch_data.right ** 2),
    d: Math.sqrt(element_data.touch_data.top ** 2 + element_data.touch_data.left ** 2)
  };
}
